...
| crafting | uses the saved Quick Craft preset, fills affordable slots, and persists collection/refill timers; first natural collection and empty-slot verification passed |
...
| scheduling | optional daily pack checks, Cafe visits and per-slot crafting deadlines, bounded failure retries, optional game closure between visits |
...
capture an actual school rank-up, repeat Cafe visits after cooldown, and finish the free-invitation check. configured mission sweeps are a useful next job: collecting Cafe AP clears its storage, but spending it needs its own policy. Club validation, crafting refill checks, and event farming follow one verified routine at a time.
