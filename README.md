...
| scheduling | optional daily routine after Global reset with saved occurrence state, hourly AP checks, daily pack checks, Cafe visits and per-slot crafting deadlines, optional game closure between visits |
...
turn on **run daily after reset** in settings to run that plan once per game day. it defaults to one minute after Global reset, with the next run shown in your local time in the queue. missed reset? it catches up on the current day when the server is back. failed or interrupted runs wait for you to inspect the log and queue a retry. keep the server running and the computer awake. [daily schedule →](docs/getting-started.md#daily-schedule)
