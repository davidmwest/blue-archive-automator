...
`daily` runs restart → club → optional packs → mail → cafe → bounties → scrimmages → lessons → optional Spend AP. club is currently a no-input placeholder, clearly marked “awaiting reset test.” the three-hour cafe schedule includes that same placeholder after restart and leaves lesson tickets alone. lessons can also be queued on their own, with a ticket limit and optional location list. it uses the tickets you already have; it never buys more. [ticket splitting and sweeps →](docs/tickets.md) [how the lesson strategies work →](docs/lessons.md) [club’s pending reset test →](docs/club.md) [crafting and saved timers →](docs/crafting.md) [paid packs and mail →](docs/packs-and-mail.md) [AP spending and rotation →](docs/spend-ap.md)
