...
`lessons` runs restart → Lessons. The full `daily` plan now includes Club, free packs, mail, Cafe, Bounties, Scrimmages, enabled Tactical Challenge battles followed by rewards, Lessons, and Tasks rewards, with configurable daily inclusion for ticket/lesson jobs and opt-in paid packs and AP spending. All successful plans finish with a home-notification scan. The three-hour Cafe timer checks Club and mail but does not spend lesson tickets. A completed Cafe visit still updates its schedule if later Lessons work fails or is interrupted. The optional daily-reset schedule now queues the full Daily plan, including Lessons when enabled. There is no independent Lessons timer or lesson-ticket purchase flow.
...
Tactical Challenge collects available rewards and verifies that battle tickets are unchanged. Time Reward collection has passed live. A live Daily Reward was collected and its exact receipt recovered after a freshness timeout; the corrected reader has offline regression coverage. A separate ladder job adds bounded opponent scouting, weighted team-level selection, one battle per opponent per game day, saved-team completion, and a configurable ticket reserve. Its battle validation status is documented separately from reward claims. See [Tactical Challenge](tactical-challenge.md).
