...
## Rank and points rewards

reward collection is a separate job: `ba --config config/local.toml assault_rewards`. it checks **Rank Reward** and **Total Points Reward**, claims whatever is available, records the receipt in Loot Gathered, and returns home. it never enters a battle or spends a raid ticket. a disabled claim is left alone; the current season's rank reward says it becomes available on September 29, so points can be claimed while rank rewards are still pending.

the job runs after Total Assault and in Daily even when the combat switch is off. the notification scan also opens Campaign after each successful job, requires the Total Assault red dot in two observations, and queues this collector once per busy queue batch. it returns home before publishing the request. amber availability markers do not trigger collection, and a notification can never enable the ticket-spending Total Assault task. Tactical Challenge uses the same Campaign red-dot check for its existing rewards-only job.

both reward categories have one claim attempt per visit. a complete receipt is required before the task reports successful collection or dismisses it. partial observations are saved for review; the optional idle-close setting can still close the game after a failed job. the job never repeatedly presses Claim. both tabs are checked without changing the observed raid-ticket balance.
...
| Rank and points collection | Rank Reward was observed disabled; Total Points Reward was claimed once. Its long receipt required recognition fixes and recovery before all **12 item types** were verified, including **3,000,000 credits, 200 Pyroxenes, and 40 Eligma**. Recovery updated the original receipt rather than adding another claim. The points claim was then verified disabled, raid tickets remained **0**, and return home was verified. This validates recovery of the claimed receipt; a fresh uninterrupted two-tab collection still needs live coverage, as does an available rank reward. |
