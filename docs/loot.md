...
Café and Lessons let the opening celebration animate for three seconds before capturing its details. They then wait for two matching rank/stat readings, within an eight-second window (plus any in-flight OCR). Only that later frame becomes the saved receipt. If details remain unreadable, the observed partial result is kept; a missing name alone never prolongs the wait. The journal records whether the details settled. Commas between stat bonuses are supported. Offline regressions cover delayed details, partial results, duplicate celebrations, and fresh-screen checks before dismissal. Historical receipts captured before the details appeared cannot recover those values from the early screenshot.
