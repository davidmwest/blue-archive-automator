...
During initial horizontal edge normalization, perspective can produce a small vertical correction near a corner. Up to two corrections of at most 40 pixels are accepted only when the camera also moves in the commanded direction. Each corrected view is checked for students, accumulated distance and boundary evidence are discarded, and two fresh stationary drags are still required. Normal row traversal, backward movement, larger drift, and further corrections retain the unexpected-movement stop. A sanitized replay of the September 24 floor-two failure verifies the measured 60-pixel horizontal / 31-pixel vertical correction; the recovery itself has offline coverage and still needs a live occurrence.
