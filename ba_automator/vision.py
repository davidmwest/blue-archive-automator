...
def classify(words: list[Word], matches: dict[str, tuple[int, int]]) -> Observation:
    ...
    # The current day label fades during its stamp; tolerate any one missing day.
    def at(label, bounds):
        x1, y1, x2, y2 = bounds
    ...
    if (at("aronasattendance", (580, 105, 1120, 177)) and
        ...
        sum(at(f"day{n}", (480+(n-1)%5*154, 174+(n-1)//5*218,
                            590+(n-1)%5*154, 218+(n-1)//5*218)) for n in range(1, 11)) >= 9):
